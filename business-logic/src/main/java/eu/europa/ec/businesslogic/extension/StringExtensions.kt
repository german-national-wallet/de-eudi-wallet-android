...
fun String.toHashedStringWithSha256(): String {
...
/**
 * Cleans a PEM-encoded key string by removing newlines, carriage returns,
 * and leading/trailing spaces.
 *
 * This function ensures the PEM content is in a single-line format, which is
 * often required for proper Base64 decoding and cryptographic operations.
 *
 * @receiver The raw PEM string that may contain newlines or extra whitespace.
 * @return A cleaned PEM string with all newlines and extra spaces removed.
 * **/
fun String.cleanPem(): String {
    return this.replace("\n", "")
        .replace("\r", "")
        .trim()
}
...
fun String.toTitleCase(): String =
    lowercase().split(" ").joinToString(" ") { it.replaceFirstChar(Char::titlecaseChar) }
