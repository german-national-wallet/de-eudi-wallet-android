...
/**
 * Converts Byte Array to String using UTF-8 character set
 *
 * @return String object
 */
fun ByteArray.toStringUsingUtf8() = String(this, Charsets.UTF_8)


/**
 * Converts a [ByteArray] into a hexadecimal string representation.
 *
 * Each byte in the array is converted to a two-character lowercase hexadecimal value,
 * and the resulting string contains these values concatenated without any separators.
 *
 * Example:
 * ```
 * val byteArray = byteArrayOf(0x0F, 0xAB.toByte(), 0x3)
 * println(byteArray.toHexString()) // Output: "0fab03"
 * ```
 *
...
fun ByteArray.toHexString(): String {
    val hexFormat = "%02x"
    return this.joinToString(separator = "") { hexFormat.format(it) }
}
