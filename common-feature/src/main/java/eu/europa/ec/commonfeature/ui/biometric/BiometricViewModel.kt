...
import eu.europa.ec.commonfeature.interactor.BiometricPinValidationResult
...
sealed class Event : ViewEvent {
    ...
    data object OnQuickPinEntered : Event()
}
...
data class State(
    ...
    val quickPinSize: Int = CodeLength.WALLET_PIN,
    val pinState: CodeEntryState = CodeEntryState(CodeEntryBuffer(quickPinSize)),
) : ViewState
...
class BiometricViewModel(
    ...
    private fun authorizeWithPin() {
        ...
        val pin = entered.consumeAsPin()
        viewModelScope.launch {
            biometricInteractor.isPinValid(pin)
                .collect {
                    when (it) {
                        is BiometricPinValidationResult.Failed -> {
                            setState {
                                copy(
                                    pinState = pinState.cleared(supportingText = it.errorMessage)
                                )
                            }
                        }

                        is BiometricPinValidationResult.Success -> {
