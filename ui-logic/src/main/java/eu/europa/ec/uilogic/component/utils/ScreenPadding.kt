...
enum class TopSpacing {
    WithToolbar, WithoutToolbar, None
}
...
internal fun stickyBottomPaddings(
    ...
    return PaddingValues(
        ...
        // no top because we would handle from the main view
    )
}
...
/**
 * Handy function to give padding to first and last items in a list
 */
@Composable
fun paddingForFirstAndLastItems(index : Int, arraySize: Int, space : Dp): Modifier {
    return when (index) {
        0 -> Modifier.padding(
            top = space
        )
        arraySize - 1 -> Modifier.padding(bottom = SPACING_MEDIUM.dp)
        else -> Modifier
    }
}