...
enum class LibraryModule(val path: String) {
    ...
    Core(":core:wallet-core"),
    CoreDocumentManager(":core:document-manager"),
    CoreTransferManager(":core:transfer-manager"),
    ...
    RevocationFeature(":revocation-feature"),
    DashboardFeature(":dashboard-feature"),
    PresentationFeature(":presentation-feature"),
    CardReaderFeature(":card-reader-feature"),
    IssuanceFeature(":issuance-feature"),
    WalletPinFeature(":wallet-pin-feature"),
    PushNotificationsFeature(":push-notifications-feature"),
    FeatureFlags(":feature-flags");

    val isLogicModule: Boolean
        get() {
            return this.name.contains("Logic") || this.name.contains("Core")
        }
